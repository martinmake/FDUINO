...
namespace Driver
{
	...
	void LineSensor::sense_line(void)
	{
		sample_line();
		...
		m_average = 0.0;
		for (uint8_t i = 0; i < LINE_SENSOR_COUNT; i++)
			m_average += m_sensor_values[i];
		m_average /= LINE_SENSOR_COUNT;

		m_is_on_line = m_average > LINE_SENSOR_THRESHOLD_AVERAGE;

		m_position = 0.0;
		for (uint8_t i = 0; i < LINE_SENSOR_COUNT; i++)
			m_position += m_sensor_values[i] * m_weights[i];
	}
}
